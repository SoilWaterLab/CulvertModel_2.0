...
def calculate(sorted_filename, watershed_precip_input_filename, rainfall_adjustment, output_filename, skipped_filename = False):
    ...
    #if len(skipped_watersheds) > 0 & (skipped_filename != False):
    #   with open(skipped_filename, 'wb') as output_file:
    #    csv_writer = csv.writer(output_file)

        # Header
    #    csv_writer.writerow(['BarrierID', 'Area_sqkm', 'Tc_hr', 'CN', 'reason'])
     #   for skipped in skipped_watersheds:
    #        csv_writer.writerow(watershed)
			##  Fix this - 162 - 168
